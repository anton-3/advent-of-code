    ...
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 100; j++) {
            for (int k = 0; k < 100; k++) {
                ...
                int i4 = 100-i1-i2-i3;
                int c = i1*capacity[0] + i2*capacity[1] + i3*capacity[2] + i4*capacity[3];
                int d = i1*durability[0] + i2*durability[1] + i3*durability[2] + i4*durability[3];
                int f = i1*flavor[0] + i2*flavor[1] + i3*flavor[2] + i4*flavor[3];
                int t = i1*texture[0] + i2*texture[1] + i3*texture[2] + i4*texture[3];
                if (c < 0) c = 0;
                if (d < 0) d = 0;
                if (f < 0) f = 0;
                if (t < 0) t = 0;
                int score = c * d * f * t;
                int cals = i1*calories[0] + i2*calories[1] + i3*calories[2] + i4*calories[3];
                if (score > maxScore && cals == 500) maxScore = score;
            }
        }
    }
    printf("%d\n", maxScore);

...
int recurse(int *array, int size, int n, bool *indices, bool *visited) {
    int hash = 0;
    for (int i = 0; i < size; i++) {
        if (indices[i]) hash ^= two[i];
    }
    ...
    for (int i = 0; i < size; i++) {
        ...
        indices[i] = false;
        count += recurse(array, size, n, indices, visited);
        indices[i] = true;
    }
    return count;
}
...
int main(int argc, char **argv) {
    ...
    int result = recurse(sizes, numLines, n, indices, visited);
    printf("%d\n", result);
    return 0;
}

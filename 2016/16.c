...
void fuck(char *data) {
    int lineLen = strlen(data);
    data[lineLen] = '0';
    for (int i = 0; i < lineLen; i++) {
        data[lineLen+i+1] = data[i] == '1' ? '0' : '1';
    }
}

void checksum(char *data) {
    int dataLen = strlen(data);
    char *checksum = (char *) malloc((1+dataLen/2) * sizeof(char));
    ...
    strcpy(data, checksum);
    free(checksum);
}

void part1(char **lines, int numLines) {
    int size = 35651584;
    char *data = (char *) malloc(100000000 * sizeof(char));
    data[0] = '\0';
    strcat(data, "01110110101001000");

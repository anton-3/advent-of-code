 ...
 */
int dijkstra(char **grid, int startX, int startY, int numGoalNodes) {
    return 0;
}

int main(int argc, char **argv) {
    char *filename = "sample24.txt";
    if (argc >= 2) {
        filename = argv[1];
    }
    int numLines;
    ...
    for (int i = 0; i < numLines; i++) {
        for (int j = 0; j < lineLength; j++) {
            ...
            if (square != '.' && square != '#') numGoalNodes++;
        }
    }
    printf("starting at x=%d, y=%d, and searching for %d locations\n", startX, startY, numGoalNodes);

    int *distances = getDistancesBFS(lines, startX, startY, numGoalNodes);

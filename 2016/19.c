...
int main(int argc, char **argv) {
    int num = 3014387;
    int *elves = (int *) malloc(num * sizeof(int));
    for (int i = 0; i < num; i++) {
        elves[i] = i;
    }
    int elf = -1;
    ...
    while (len > 1) {
        ...
        if (elf == num) elf = 0;
        elfI = 0;
        while (elves[elfI] != elf && elfI != num) {
            elfI++;
        }
        if (elfI == num) continue;
        nextI = (elfI + len/2) % len;
        len--;
        if (len % 100000 == 0) printf("%d\n", len);
        for (int i = nextI; i < len; i++) {
            elves[i] = elves[i+1];
        }
    }
    printf("%d\n", elves[0]+1);
